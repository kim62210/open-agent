...
if TYPE_CHECKING:
    from open_agent.models.job import CreateJobRequest, UpdateJobRequest
...
def validate_job_prompt(prompt: str) -> str | None:
    """프롬프트 안전 검증. 문제가 있으면 에러 메시지 반환, 유효하면 None."""
    if not prompt or not prompt.strip():
        return "프롬프트가 비어 있습니다."
    if len(prompt) > MAX_PROMPT_LENGTH:
        return (
            f"프롬프트가 너무 깁니다 ({len(prompt)}자). 최대 {MAX_PROMPT_LENGTH}자까지 허용됩니다."
        )
...
class JobManager:
    ...
    async def _persist_job(self, job: JobInfo) -> None:
        ...
        async with async_session_factory() as session:
            orm = JobORM(
                id=job.id,
                owner_user_id=self._owners.get(job.id),
                name=job.name,
                ...
                if isinstance(job.schedule_type, str)
                else job.schedule_type.value,
                ...
                if isinstance(job.last_run_status, (str, type(None)))
                else (job.last_run_status.value if job.last_run_status else None),
    ...
    def get_all_jobs(self, owner_user_id: str | None = None) -> list[JobInfo]:
        return [
            job
            for job_id, job in self._jobs.items()
            if owner_user_id is None or self._owners.get(job_id) == owner_user_id
        ]

    def get_job(self, job_id: str, owner_user_id: str | None = None) -> JobInfo | None:
        if owner_user_id is not None and self._owners.get(job_id) != owner_user_id:
            return None
        return self._jobs.get(job_id)

    async def create_job(
        self, req: "CreateJobRequest", owner_user_id: str | None = None
    ) -> JobInfo:
        async with self._lock:
            ...
            self._owners[job_id] = owner_user_id
            await self._persist_job(job)
            logger.info(f"Created job: {req.name} ({job_id})")
            return job

    async def update_job(
        self, job_id: str, req: "UpdateJobRequest", owner_user_id: str | None = None
    ) -> JobInfo | None:
        async with self._lock:
    ...
    async def delete_job(self, job_id: str, owner_user_id: str | None = None) -> bool:
        async with self._lock:
    ...
    async def toggle_job(self, job_id: str, owner_user_id: str | None = None) -> JobInfo | None:
        async with self._lock:
            job = self._jobs.get(job_id)
            if not job:
                return None
    ...
    async def start_run(self, job_id: str) -> str | None:
    ...
    def get_run_history(
        self, job_id: str, limit: int = 20, owner_user_id: str | None = None
    ) -> list[JobRunRecord]:
    ...
    async def set_next_run_at(self, job_id: str, next_run: str | None) -> None:
        async with self._lock:
            job = self._jobs.get(job_id)
            if not job:
                return
    ...
    async def get_job_tools(self) -> list[dict[str, Any]]:
        ...
        servers = [
            s for s in mcp_manager.get_all_server_statuses() if s.status.value == "connected"
        ]
        mcp_info = ""
        if servers:
            lines = [
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "name": {
                                "type": "string",
                                "description": "작업 이름 (간결하게, 예: '뉴스 요약', '서버 상태 체크')",
                            },
                            "prompt": {
                                "type": "string",
                                "description": "실행할 프롬프트 — AI 에이전트에게 전달될 지시 내용",
                            },
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "job_id": {
                                "type": "string",
                                "description": "삭제할 작업 ID (id 또는 name 중 하나 필수)",
                            },
                            "name": {
                                "type": "string",
                                "description": "삭제할 작업 이름 (id 또는 name 중 하나 필수)",
                            },
                        },
                    },
                },
        ...
        try:
            ...
            for sn in skill_names:
                ...
                if skill and skill.allowed_tools:
                    auto_mcp_servers.update(_extract_mcp_servers_from_tools(skill.allowed_tools))
            ...
            is_scheduled_once = req.schedule_type == "once" and req.schedule_config.get("run_at")
    ...
    def _handle_list_jobs(self) -> str:
        ...
        for job in jobs:
            ...
            lines.append(f"- [{job.id}] {job.name} | {schedule} | {status} | 마지막: {last_run}")
        return "\n".join(lines)
    ...
    async def _handle_delete_job(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.warning(f"Failed to stop running job {job_id}", exc_info=exc)
    ...
    async def _get_server_tool_names(server_name: str, mcp_manager) -> list[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to get tool names for server '{server_name}'", exc_info=exc)
            return []
