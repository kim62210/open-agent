...
async def create_memory(req: CreateMemoryRequest, current_user: Annotated[dict, Depends(require_user)]):
    return await memory_manager.create(content=req.content, category=req.category, confidence=req.confidence)


@router.patch("/{memory_id}", response_model=MemoryItem)
async def update_memory(memory_id: str, req: UpdateMemoryRequest, current_user: Annotated[dict, Depends(require_user)]):
    updated = await memory_manager.update(memory_id, **req.model_dump(exclude_unset=True))
    if not updated:
...
async def toggle_pin(memory_id: str, current_user: Annotated[dict, Depends(require_user)]):
    ...
    if not mem:
        raise HTTPException(status_code=404, detail="Memory not found")
    updated = await memory_manager.update(memory_id, is_pinned=not mem.is_pinned)
    return updated
