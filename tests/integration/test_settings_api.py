...
async def settings_client(_patch_db_factory, monkeypatch):
    ...
    _DEFAULT = {
        "llm": {"model": "gemini/gemini-2.0-flash", "temperature": 0.7, "max_tokens": 16384, "system_prompt": ""},
        "memory": {"enabled": True, "max_memories": 50, "max_injection_tokens": 2000,
                    "compression_threshold": 0.8, "extraction_interval": 3},
        "profile": {"name": "", "avatar": "", "platform_name": "Open Agent",
                     "platform_subtitle": "Open Agent System", "bot_name": "Open Agent Core", "bot_avatar": ""},
        "theme": {"accent_color": "amber", "mode": "dark", "tone": "default", "show_blobs": True,
                   "chat_bg_image": "", "chat_bg_opacity": 0.3, "chat_bg_scale": 1.1,
                   "chat_bg_position_x": 50, "chat_bg_position_y": 50, "font_scale": 1.0},
    }
...
class TestPatchLLMSettings:
    ...
    async def test_update_temperature(self, settings_client: AsyncClient):
        ...
        from open_agent.core.settings_manager import settings_manager as _stm
        _stm.update_llm = AsyncMock(return_value=_stm._settings.llm.model_copy(update={"temperature": 0.9}))
        resp = await settings_client.patch(
            "/api/settings/llm", json={"temperature": 0.9}
        )
        assert resp.status_code == 200
...
class TestMemorySettings:
    ...
    async def test_update_memory_settings(self, settings_client: AsyncClient):
        ...
        from open_agent.core.settings_manager import settings_manager as _stm
        _stm.update_memory = AsyncMock(return_value=_stm._settings.memory.model_copy(update={"max_memories": 100}))
        resp = await settings_client.patch(
            "/api/settings/memory", json={"max_memories": 100}
        )
        assert resp.status_code == 200
...
class TestProfileSettings:
    ...
    async def test_update_profile(self, settings_client: AsyncClient):
        ...
        from open_agent.core.settings_manager import settings_manager as _stm
        _stm.update_profile = AsyncMock(return_value=_stm._settings.profile.model_copy(update={"name": "TestBot"}))
        resp = await settings_client.patch(
            "/api/settings/profile", json={"name": "TestBot"}
        )
        assert resp.status_code == 200
...
class TestThemeSettings:
    ...
    async def test_update_theme(self, settings_client: AsyncClient):
        ...
        from open_agent.core.settings_manager import settings_manager as _stm
        _stm.update_theme = AsyncMock(return_value=_stm._settings.theme.model_copy(update={"accent_color": "blue"}))
        resp = await settings_client.patch(
            "/api/settings/theme", json={"accent_color": "blue"}
        )
        assert resp.status_code == 200
...
class TestValidateModel:
    ...
    async def test_validate_model_failure(self, settings_client: AsyncClient):
        ...
        with patch("open_agent.api.endpoints.settings.acompletion", new_callable=AsyncMock, side_effect=RuntimeError("bad model")):
            with patch("open_agent.api.endpoints.settings.LLMClient") as mock_llm:
                mock_llm._resolve_api_key = MagicMock(return_value=None)
                resp = await settings_client.post(
                    "/api/settings/validate-model",
